...
public class ReservationEventPublisher {
    ...
    public void onReservationCreated(ReservationCreatedDomainEvent event) {
        try {
            availableTimeRepository.findById(event.availableTimeId()).ifPresent(at ->
                    resourceRepository.findById(event.resourceId()).ifPresent(resource -> {
                        int occupied = reservationRepository.sumHeadCountByAvailableTimeId(event.availableTimeId());
                        if (occupied >= resource.getMaxCapacity()) {
                            at.block();
