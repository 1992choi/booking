...
| API 문서 | Swagger UI용 JWT Bearer 시큐리티 스킴 전역 등록 | springdoc 의존성 자체(각 서비스가 필요 시 추가) |
...
└── src/main/java/com/example/booking/core/
    ...
    └── openapi/
        └── OpenApiAutoConfiguration.java (@AutoConfiguration — springdoc OpenAPI 클래스패스에 있을 때만. JWT Bearer 시큐리티 스킴을 전역 등록해 Swagger UI에 Authorize 버튼 노출)
```
...
각 서비스가 core 를 임베드하면 아래 6개 `@AutoConfiguration` 이 자동 등록된다 (`META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports`).
...
| `OpenApiAutoConfiguration` | springdoc `OpenAPI` 클래스패스에 있을 때 | JWT Bearer 시큐리티 스킴이 등록된 `OpenAPI` 빈 (Swagger UI Authorize 버튼) |